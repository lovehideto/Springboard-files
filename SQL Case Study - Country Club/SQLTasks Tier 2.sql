...
SELECT COUNT(name) AS no_fee_count
FROM `Facilities` 
WHERE membercost=0.0;
...
SELECT facid, name,monthlymaintenance,membercost
FROM `Facilities` 
WHERE membercost>0.0
...
SELECT * 
FROM (SELECT f.name, concat(m.firstname,' ',m.surname) AS member_name, 
      CASE b.memid
      ...
      INNER JOIN Members AS m
      ON b.memid = m.memid
      WHERE starttime LIKE '2012-09-14%') AS subqurey
WHERE cost>30
ORDER BY cost DESC;
